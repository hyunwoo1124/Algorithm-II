...
bool is_decreasing(const sequence& A) {
  for (size_t i = 1; i < A.size(); ++i) {
      ...
      if(A[i-1] < A[i]){          // [0] [1] [2] [3] [4]
        return false;             //  5   7
      }                           // if elem 1 is bigger than elem 0
  }                               // its not in decreasing order so return false
  return true;                    // if thats not the case return true;
}
  ...
  for (signed int i = n-2;  i>= 0; i--) {
    for (size_t j = i+1; j < n ; j++) {
        ...
        if(A[j] < A[i] && H[j] >= H[i]){
          /*
          inter         i   j
          A     5  10  40  20     for example if i A[i] > A[j]
          H     0   0   0   0     that means its in decreasing order so h will be added one
          */
          H[i] = H[j] +1;
        }
    }
...
sequence longest_decreasing_powerset(const sequence& A) {
  ...
  while (true) {
   ...
   // TODO
      ...
      if(is_decreasing(candidate) == true && candidate.size() > best.size()){
        best = candidate;
        // if decreasing order is true and candidate size is bigger than best return biggest
      }
